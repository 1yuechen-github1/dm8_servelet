package org.example.Controller.Teacher.Cource;
...
//        Object object1 = objectService.queryByObjectId(object.getId());
     ...
     * 找到每个课程对应的班级列表
     * **/
    @GetMapping("/queryClassBySubject")
    public R queryClassBySubject(@RequestParam int subjectId){
        List<ClassRoomObject> classRooms = objectService.queryClassBySubject(subjectId);
        ArrayList<ClassRoom> classRooms1=new ArrayList<>();
        for (ClassRoomObject classRoomObject : classRooms) {
            int classesId = classRoomObject.getClassesId();
            ClassRoom classRoom= classRoomService.queryStudentById(classesId);
            classRooms1.add(classRoom);
        }
        return R.success(classRooms1);
    }
}
